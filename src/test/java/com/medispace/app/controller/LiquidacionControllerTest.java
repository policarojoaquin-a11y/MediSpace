...
class LiquidacionControllerTest {
    ...
    void listarPorMedico_MedicoPidiendoIdDeOtroMedicoRecibe403() {
        ...
        verify(liquidacionService, never()).listarLiquidacionesPorMedico(any());
    }

    @Test
    void listarPorMedico_MedicoPidiendoSuPropioIdRecibeSusDatos() {
        Authentication auth = authMedico();
        when(liquidacionService.listarLiquidacionesPorMedico(1))
                .thenReturn(List.of(LiquidacionResponseDTO.builder().idLiquidacion(1).build()));
